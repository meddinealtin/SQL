...
--subquery'si nedir : select max(memur_sayisi) from sirketler;
...
--subquery'si nedir : select min(memur_sayisi)*1.5 from sirketler;
...
--subquery'si nedir : select sum(memur_sayisi) from sirketler where sirket_ismi in('Nokia','xiaomi');
...
--subquery'si nedir : select avg(memur_sayisi)from sirketler 1800.0000000000000000
...
--SELECT sirket_ismi FROM sirketler s  WHERE memur_sayisi > 2200


--SELECT ad, calisma_performanslari FROM memurlar m 
--JOIN sirketler s ON m.sirket_ismi = s.sirket_ismi 
--WHERE m.sirket_ismi = (SELECT sirket_ismi FROM sirketler s  WHERE memur_sayisi > 2200)


select sirket_ismi from sirketler where memur_sayisi>1899;

select ad,calisma_performanslari from memurlar where sirket_ismi in(select sirket_ismi from sirketler where memur_sayisi>2200);


-- SORU6: Ankara'da memurlari olan sirketlerin 
--sirket id'lerini ve memurlar sayilarini listeleyiniz

SELECT sirket_ismi FROM memurlar m WHERE sehir = 'Ankara'

SELECT sirket_id, memur_sayisi FROM sirketler s WHERE sirket_ismi IN (SELECT sirket_ismi FROM memurlar m WHERE sehir = 'Ankara')
...
--SELECT sirket_ismi FROM memurlar WHERE sehir = 'Giresun'

--SELECT sirket_ismi FROM sirketler 
--WHERE EXISTS (SELECT sirket_ismi FROM memurlar WHERE sehir = 'Giresun')
